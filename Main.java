...
public class Main {	
	public static void main(String[] args) {
		...
		Evento ev1 = new EventoSemana("Semana Acadêmica IFRS", "Rio Grande");

		// atividades tem vagas e carga horária opcionais
		Atividade atv1 = ev1.novoSeminario("Seminário TCCs TADS", 40);
		MiniCurso atv2 = (MiniCurso) ev1.novoMinicurso("Minicurso ECMA6", 20, 8);

		// dados do evento
		System.out.println(ev1.getNome().equals("Semana Acadêmica IFRS"));
		System.out.println(ev1.getTipo().equals("Semana"));
		...
		System.out.println(atv1.getTipo().equals("Seminario"));
		...
		System.out.println(atv2.getTipo().equals("MiniCurso"));
		...
		Evento ev2 = new EventoEscola("Escola Regional de Banco de Dados", "Bagé"); // Escola
		Oficina atv3 = (Oficina) ev2.novaAtividade("BigData com Apache Hadoop", "Paulo Silva", 20, 4); //Oficina
		Atividade atv4 = ev2.novaAtividade("Data Science: uma introdução", "Maria Santos"); // Palestra
		...
		System.out.println(atv3.getTipo().equals("Oficina"));

		// um evento que abriga (tem) outros eventos:
		Evento ev3 = new EventoSalao("3º Salão de Pesquisa, Extensão e Ensino do IFRS", "Bento Gonçalves");
		EventoSatelite sat1 = ev3.novoEventoSeminario("7º Seminário de Iniciação Científica e Tecnológica (SICT)");
		EventoSatelite sat2 = ev3.novoEventoSeminario("5º Seminário de Educação Profissional e Tecnológica (SEMEPT)");
		EventoSatelite sat3 = ev3.novoEventoMostra("Mostra de Arte e Cultura");
		...
		for (int i = 0; i < insc1.getChave().toString().length(); i++) {
		...
		System.out.println(atv4.getTipo().equals("Palestra"));
